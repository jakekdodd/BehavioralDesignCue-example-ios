...
class DataViewController: UIViewController {
    ...
    @IBOutlet weak var cueSourceLabel: UILabel!
    var cueSource: String {
        if let delegate = UIApplication.shared.delegate as? AppDelegate,
            let cue = delegate.cue {
            return "\(cue.source.description)"
        } else {
            return "unknown cue"
        }
    }
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(updateViews), name: .UIApplicationDidBecomeActive, object: nil)
    }
    
    override func viewWillAppear(_ animated: Bool) {
        super.viewWillAppear(animated)
        self.dataLabel!.text = dataObject
        self.cueSourceLabel.text = cueSource
        self.logView.text = logObject
    }
    
    @objc func updateViews() {
        DispatchQueue.main.async {
            self.cueSourceLabel.text = self.cueSource
    ...
    @IBAction func scheduleNotification(_ sender: Any) {
        if #available(iOS 10.0, *) {
            UNUserNotificationCenter.current().requestPermission() { granted in
                ...
                UNUserNotificationCenter.current().scheduleNotification(identifier: "test", body: "Body", time: 1)
                UIApplication.shared.performSelector(onMainThread: #selector(URLSessionTask.suspend), with: nil, waitUntilDone: false)
            }
        } else {
    ...
    @IBAction func clearLogObject(_ sender: Any) {
        let clearAlert = UIAlertController(title: "Log", message: "Do you want to clear the log history?", preferredStyle: UIAlertControllerStyle.alert)
